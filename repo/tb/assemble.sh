...
output_file="$SCRIPT_DIR/program.hex" #giving path of output file
PROGRAM_FOLDER=$(realpath "$SCRIPT_DIR/../rtl/program.hex") 
...
#-t is to specify the output format, x1 means x = hexadecimal and 1 is the size specifier here its 1 which represents the byte count, so we will
#output the hex two hex digits at a time
